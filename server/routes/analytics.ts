...
// Алгоритм Лінійної Регресії (y = mx + b)
function calculateLinearRegression(dataPoints: number[]) {
    const n = dataPoints.length;
    if (n === 0) return 0;
    if (n === 1) return dataPoints[0]; // Якщо є дані лише за 1 місяць, прогнозуємо таку ж суму

    let sumX = 0, sumY = 0, sumXY = 0, sumXX = 0;

    for (let i = 0; i < n; i++) {
        const x = i + 1; // Час (1-й місяць, 2-й місяць і т.д.)
        const y = dataPoints[i]; // Сума витрат
    ...
    // Знаходимо коефіцієнт нахилу (m) та перетин (b)
    ...
    // Прогнозуємо для наступного місяця (x = n + 1)
    const nextX = n + 1;
    const prediction = m * nextX + b;

    // Витрати не можуть бути від'ємними
